...
async function handleToolCall(name, args) {
    switch (name) {
        ...
        case "auth_login": {
            ...
            const result = await apiRequest("/api/Auth/login", "POST", request);
        ...
        case "create_tasks_bulk": {
            ...
            const result = await apiRequest("/api/TaskBulk", "POST", tasks);
            return JSON.stringify(result, null, 2);
        }
        case "get_bulk_task_status": {
            const taskId = args.taskId;
            const result = await apiRequest(`/api/TaskBulk/${encodeURIComponent(taskId)}`, "GET");
            return JSON.stringify(result, null, 2);
        }
        // Utility
        case "set_auth_token": {
